...
/*
 * triggered_change_notification_json
 *
 * This trigger function will send a notification of data modification with
 * primary key values.  The channel will be "tcn_json" unless the trigger is
 * created with a parameter, in which case that parameter will be used.
 */
PG_FUNCTION_INFO_V1(triggered_change_notification_json);

Datum
triggered_change_notification_json(PG_FUNCTION_ARGS)
{
	...
	if (!CALLED_AS_TRIGGER(fcinfo))
		...
		errmsg("triggered_change_notification_json: must be called as trigger")));

	/* and that it's called after the change */
	if (!TRIGGER_FIRED_AFTER(trigdata->tg_event))
		ereport(ERROR,
				(errcode(ERRCODE_E_R_I_E_TRIGGER_PROTOCOL_VIOLATED),
				 errmsg("triggered_change_notification_json: must be called after the change")));

	/* and that it's called for each row */
	if (!TRIGGER_FIRED_FOR_ROW(trigdata->tg_event))
		ereport(ERROR,
				(errcode(ERRCODE_E_R_I_E_TRIGGER_PROTOCOL_VIOLATED),
				 errmsg("triggered_change_notification_json: must be called for each row")));
	...
	{
		elog(ERROR, "triggered_change_notification_json: trigger fired by unrecognized operation");
	...
	if (nargs > 2)
		ereport(ERROR,
				(errcode(ERRCODE_E_R_I_E_TRIGGER_PROTOCOL_VIOLATED),
				 errmsg("triggered_change_notification_json: must not be called with more than 2 parameter")));
	...
	if (nargs == 0){
		...
		channel = trigger->tgargs[0];		
		strcpy_quoted(payload, trigger->tgargs[1], '"');	
	}else{
		channel = "tcn_json";
	}
	...
	if (!foundPK)
		ereport(ERROR,
				(errcode(ERRCODE_E_R_I_E_TRIGGER_PROTOCOL_VIOLATED),
				 errmsg("triggered_change_notification_json: must be called on a table with a primary key")));
